...
import { lucideSettings, lucideZap } from '@ng-icons/lucide'
  ...
  template: `
    <header class="flex h-14 items-center gap-4 border-b px-4">
      <div class="flex items-center gap-2 font-semibold">
        ...
        >
          <path
            ...
            fill="url(#header_logo_gradient)"
          />
          <defs>
            <linearGradient
              id="header_logo_gradient"
              x1="255.628"
              y1="-34.3245"
              x2="618.483"
              y2="632.032"
        ...
        @if (store.serverVersion) {
          <span class="text-xs text-muted-foreground ml-1"> v{{ store.serverVersion }} </span>
        }

        @if (store.oaiMode.enabled) {
          <app-badge variant="secondary" class="gap-1 ml-2">
            <ng-icon name="lucideZap" class="h-3 w-3" />
            OpenAI: {{ store.oaiMode.model }}
          </app-badge>
        }
      </div>
      ...
      <div class="flex items-center gap-2 ml-auto">
        <app-mode-toggle />
        <button [appButton] variant="ghost" size="sm" (click)="handleSettingsClick($event)">
...
export class AppHeaderComponent {
  ...
  agents = input.required<AgentInfo[]>()
  workflows = input.required<WorkflowInfo[]>()
  entities = input<(AgentInfo | WorkflowInfo)[]>([])
  selectedItem = input<AgentInfo | WorkflowInfo>()
  isLoading = input<boolean>(false)

  // Outputs using Signal API
  onSelect = output<AgentInfo | WorkflowInfo>()
  onBrowseGallery = output<void>()
  onSettingsClick = output<void>()
